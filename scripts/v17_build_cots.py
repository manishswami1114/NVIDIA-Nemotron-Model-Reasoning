...
Steps:
  1. (Pre-step done by hand) `all_categorical_splits_v17/` already has 7 files
...
def main():
    ...
    for sid, sol in sols.items():
        ...
        try:
            content = W.write_cryptarithm_deductive(puzzle)
    ...
    print("=" * 70)
    write_jsonl(V17 / "train_cot_cryptarithm_deduce.jsonl", crypto_deduce)
    write_jsonl(V17 / "train_cot_cryptarithm_guess.jsonl",  crypto_guess)
    ...
    print("=" * 70)
    total = 0
    for fp in sorted(V17.glob("train_cot_*.jsonl")):
