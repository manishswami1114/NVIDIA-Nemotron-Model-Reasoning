...
RuleFamily = Literal[
    ...
    "OR-NOT",
    "DEFAULT",
    "COMPOSITE"  # Added for 3-operand fallback
]
...
def _search_composite_for_col(out_col: str, input_columns: List[str]) -> Optional[RuleCandidate]:
    """Brute-force search for 3-operand composite rules when standard 1 or 2 operand rules fail."""
    OP_PAIRS = [
        ...
        for a in range(N_BITS):
            for b in range(N_BITS):
                ...
                for c in range(N_BITS):
                    ...
                    # Optimization: halve search space for commutative inner ops
    ...
    for i in range(N_BITS):
        if i in pending_set:
            if chosen_cat and i in per_bit_cat[chosen_cat]:
                ...
                # --- NEW: COMPOSITE FALLBACK IMPLEMENTATION ---
                if all_cands:
                    lines.append(f"{i} " + " ".join(c.expr for c in all_cands))
                    best[i] = all_cands[0]
                else:
