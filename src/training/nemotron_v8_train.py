# -*- coding: utf-8 -*-
"""
Nemotron v8 — TRAINING-ONLY notebook
======================================
...
WORKFLOW:
  1. Upload train_cot_v8_merged.jsonl as a Kaggle dataset named `nemotron-cot-v8`.
...
v8 IMPROVEMENTS over v5/v6:
  - train_cot_v8_merged.jsonl = 16,100 VERIFIED examples (100% train.csv coverage)
  - Prompt-engineered CoT: step-numbered reasoning, strategy headers, verification
  - 15 CoT template variants across 6 categories
  - Curriculum learning: easy → hard ordering for better convergence
  - Optimized hyperparameters: rank 64, lr 2e-5, 3 epochs
  - Hard category upsampling (bit, transform repeated 2x)
"""
...
# ============================================================
# 4. HYPERPARAMETERS — v8 optimized
# ============================================================
LORA_RANK    = 32       # ← more capacity for 6 diverse puzzle types (was 32)
LORA_ALPHA   = 128      # ← 2:1 ratio (alpha = 2 × rank)
MAX_SEQ_LEN  = 2048
NUM_EPOCHS   = 3        # ← 3 epochs on 16K examples (was 2 on 9K)
BATCH_SIZE   = 1        # User constraint: 98GB VRAM fits batch 1
GRAD_ACCUM   = 16       # Maintain effective batch = 16 (1 × 16)
LR           = 2e-5     # ← slightly more aggressive (was 1e-5); cosine decay handles it

MODEL_PATH = "/kaggle/input/models/metric/nemotron-3-nano-30b-a3b-bf16/transformers/default/1"
OUTPUT_DIR = "/kaggle/working/adapter"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# v8 data: 16,100 verified examples with prompt-engineered CoT
# All 6 categories at 100% coverage, step-numbered reasoning, verification steps
OUR_DATA_PATHS = [
    "/kaggle/input/nemotron-cot-v8/train_cot_v8_merged.jsonl",        # primary: 16100 verified
    "/kaggle/input/nemotron-dataset/train_cot_v8_merged.jsonl",
...
# External CSV no longer needed — v8 includes all data
EXTERNAL_CSV_PATHS = [
...
# ============================================================
# 6. LOAD DATASET — with curriculum learning
# ============================================================
...
# 2) Try external CSV (fallback only)
...
if not our_data and not ext_csv_path:
    raise FileNotFoundError(
        ...
        "Upload train_cot_v8_merged.jsonl as Kaggle dataset named 'nemotron-cot-v8'"
    )

# ─── CURRICULUM LEARNING: Classify and reorder ────────────
# Strategy: easy categories first (epoch 1), then full data upsampled on hard (epochs 2-3)
# This teaches format compliance first, then focuses on harder reasoning.
HARD_CATEGORIES = {'bit manipulation', 'transformation'}

def classify_difficulty(example):
    user_msg = example['messages'][0]['content'].lower() if example.get('messages') else ''
...
if our_data:
    easy_data = [x for x in our_data if classify_difficulty(x) == 'easy']
    hard_data = [x for x in our_data if classify_difficulty(x) == 'hard']
    print(f"\nCurriculum split: {len(easy_data)} easy + {len(hard_data)} hard")

    # Curriculum order:
    # First pass: easy only (builds format compliance + basic reasoning)
    # Second pass: all data (builds all skills)
    # Third pass: mostly hard data (focuses training where it struggles most)
    import random
    random.seed(42)
    curriculum_data = []
    
    # Pass 1: easy examples (shuffled)
    pass1 = easy_data.copy()
    random.shuffle(pass1)
    curriculum_data.extend(pass1)
    
    # Pass 2: all data (shuffled)
    pass2 = our_data.copy()
    random.shuffle(pass2)
    curriculum_data.extend(pass2)
    
    # Pass 3: hard data 2x + easy data 1x (shuffled)
    # This gives Hard: 0(p1) + 1(p2) + 2(p3) = 3 total exposures
    #           Easy: 1(p1) + 1(p2) + 1(p3) = 3 total exposures... wait.
    # To truly upsample hard: let's give Hard 3x and Easy 2x.
    pass3 = hard_data.copy() * 2
    random.shuffle(pass3)
    curriculum_data.extend(pass3)
    
    our_data = curriculum_data
    print(f"  Curriculum total: {len(our_data)} examples")
    print(f"  Effective epochs on easy:  ~2x")
    print(f"  Effective epochs on hard:  ~3x")
...
steps_estimate = len(hf_dataset) // (BATCH_SIZE * GRAD_ACCUM) * 1  # 1 because SFTConfig uses num_train_epochs=1
...
LORA_TARGET_MODULES = [
    ...
    "in_proj", "out_proj",                      # Mamba-2 projections
    "up_proj", "down_proj",                     # MLP / MoE layers
    "lm_head",                                  # Crucial for learning exact formatting (like \boxed{})
]
...
training_args = SFTConfig(
    ...
    num_train_epochs=1,           # ← 1 epoch because we built 3 passes into the data
    learning_rate=LR,
    logging_steps=10,
    bf16=True,
    max_grad_norm=1.0,
    ...
    warmup_steps=50,              # ← shorter warmup (data is well-ordered)
    save_strategy="no",
    report_to="none",
    ...
    dataloader_drop_last=False,   # don't drop last batch (every example matters)
)
